...
class StudentLLMAgent(Agent):
    ...
    def __init__(self, agent_id: str, debug: bool = False, base_depth: int = 2, use_renju_rules: bool = False):
        super().__init__(agent_id)
        self.use_renju_rules = use_renju_rules
        self.debug = debug
...
""".strip()
    ...
    async def get_move(self, game_state: GameState) -> Tuple[int, int]:
        ...
        # 3. Block opponent Open Three (must-block rule)
        for move in legal_moves:
            r, c = move
            if self._match_live_three(board, r, c, opponent.value):
                return move

        # 4. Create Double Open Three
        for move in legal_moves:
            r, c = move
            if self._match_live_three(board, r, c, current_player.value) >= 2:
                return move

        # 5. Block opponent Double Open Three
        for move in legal_moves:
            r, c = move
            if self._match_live_three(board, r, c, opponent.value) >= 2:
                return move

        # 6. Create Double Closed Four
        for move in legal_moves:
            threats = self._count_threats(board, move, current_player)
            if threats >= 2 and self._match_dead_four(board, r, c, current_player.value) >= 2:
                return move

        # 7. Block opponent Double Closed Four
        for move in legal_moves:
            r, c = move
            if self._match_dead_four(board, r, c, opponent.value) >= 2:
                return move

        # 8. Four-Three ban check (if Renju rules enabled)
        if self.use_renju_rules and current_player == Player('X'):
            legal_moves = [m for m in legal_moves if not (
                self._match_live_three(board, m[0], m[1], current_player.value) >= 1 and
                (self._match_live_four(board, m[0], m[1], current_player.value) +
                self._match_dead_four(board, m[0], m[1], current_player.value)) >= 1
            )]

        # 9. Forcing moves — prioritise moves that force defence
        for move in legal_moves:
            if self._count_threats(board, move, current_player) >= 1:
                return move

        # 10. Fake threats — detect low-value threats to mislead opponent
        for move in legal_moves:
            r, c = move
            # A fake threat is a weak pattern (e.g., dead three) with no immediate win
            if self._match_dead_three(board, r, c, current_player.value) >= 1 \
            and not self._is_winning_move(board, move, current_player):
                # Ensure we have a stronger hidden threat on board
                if any(self._match_live_three(board, mr, mc, current_player.value) >= 1
                    for mr, mc in legal_moves if (mr, mc) != move):
                    return move
